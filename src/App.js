...
function SheetRecibir({open, onClose, inv, onAdd, fInv, setFInv}){
  const _sim = useState(""); const scanInvMsg = _sim[0]; const setScanInvMsg = _sim[1];
  const _sis = useState(null); const scanInvStatus = _sis[0]; const setScanInvStatus = _sis[1];
  const _br = useState(false); const barcodeReady = _br[0]; const setBarcodeReady = _br[1];
